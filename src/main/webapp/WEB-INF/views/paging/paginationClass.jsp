...
<div id="pagingDiv" class="d-flex justify-content-center mb-4">
	<div class="pagination1">
		  <span class="pagination__number-indicator"></span>
  	  	  	  ...
  	  	  	  <button type="button" onclick='location.href="<c:url value='/class/classlist/1?orgName=${orgName}&className=${className}'/>" '  id="paginBtn" style="padding: 16px 16px 16px 0px;">
				    <div class="d-flex">
			  ...
			  <c:if test="${pager.groupNo > 1}">
				  <button type="button" onclick='location.href="<c:url value='/class/classlist/${pager.startPageNo-5}?orgName=${orgName}&className=${className}'/>" '  id="paginBtn" class="pagination__arrow">
			...
			<c:forEach var="i" begin="${pager.startPageNo}" end="${pager.endPageNo}" step="1">
				<c:if test="${pager.pageNo != i}">
					<a href="<c:url value='/class/classlist/${i}?orgName=${orgName}&className=${className}'/>" id="paginBtn" class="pagination__number ">${i}</a>
				</c:if>
				<c:if test="${pager.pageNo == i}">
			...
			<c:if test="${pager.groupNo < pager.totalGroupNo}">
				  <button type="button" onclick='location.href="<c:url value='/class/classlist/${pager.startPageNo+5}?orgName=${orgName}&className=${className}'/>" '  id="paginBtn" class="pagination__arrow pagination__arrow--right">
				    <span class="pagination__arrow-half" style="background-color: #204065;"></span>
				    <span class="pagination__arrow-half" style="background-color: #204065;"></span>
				  </button>
			  </c:if>
			
			 <button type="button" onclick='location.href="<c:url value='/class/classlist/${pager.endPageNo}?orgName=${orgName}&className=${className}'/>" '   id="paginBtn" style="padding: 16px 0px 16px 16px;">
				    <div class="d-flex">
